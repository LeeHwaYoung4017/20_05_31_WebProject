			...
			<div class="all">
				<div class="delLine">
									...
									<div class="days">
										<a class="aSize" href="noticeView?idx=${vo.idx}&currentPage=${exhibitionList.currentPage}">
											...
											<h5 style="float:none; margin: 0; text-align: center; padding-top: 2%;">
												<fmt:formatDate value="${sday}" pattern="yyyy-MM-dd"/> ~ 
												<fmt:formatDate value="${eday}" pattern="yyyy-MM-dd"/>
											</h5>
										</a>
									</div>
								</li>
					...
					<div class="bottomLine">
						<!-- 이전 페이지 -->
						<c:if test="${exhibitionList.startPage > 1}">
							<input type="button" value="시작" onclick="location.href='?currentPage=1'" title="첫 페이지"/>
							<input type="button" value="이전" onclick="location.href='?currentPage=${exhibitionList.startPage -1}'">
						</c:if>
						<!-- 이전 페이지가 존재하지 않을경우 -->
						<c:if test="${exhibitionList.startPage <= 1}">
						...
						<!-- 페이지 번호 -->
						<c:forEach var="i" begin="${exhibitionList.startPage}" end="${exhibitionList.endPage }" step="1">
							<!-- 이미 선택된 페이지 -->
							<c:if test="${i == exhibitionList.currentPage }">
						...
						<!-- 다음 페이지 -->
						<c:if test="${exhibitionList.endPage < exhibitionList.totalPage}">
							<input type="button" value="다음" onclick="location.href='?currentPage=${exhibitionList.endPage + 1}'">
							<input type="button" value="마지막" onclick="location.href='?currentPage=${exhibitionList.totalPage}'">
						</c:if>
						<!-- 다음 페이지가 존재하지 않을경우 -->
						<c:if test="${exhibitionList.endPage >= exhibitionList.totalPage}">
